...
class TFBDashboard_REST_API_Order {
    public function __construct() {
        error_log('TFB Dashboard: REST API Order class initialized.'); // Debugging
        ...
        add_action('woocommerce_rest_create_order', array($this, 'tfbdashboard_save_custom_order_fields'), 10, 2);
    ...
    public function tfbdashboard_register_custom_order_fields() {
        error_log('TFB Dashboard: Registering custom order fields.'); // Debugging
    ...
    public function tfbdashboard_save_custom_order_fields($order, $request) {
        error_log('TFB Dashboard: tfbdashboard_save_custom_order_fields method called.'); // Debugging
        ...
        if (!isset($request['challengePricingId'], $request['stageId'], $request['userEmail'], $request['brandId'])) {
            error_log('TFB Dashboard: Required fields are missing.'); // Debugging
